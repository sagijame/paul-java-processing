...
public class FlockMain extends PApplet {
	...
	boolean smoothEdges = true; //false;
	...
	public void setup() {
		...
		for (int i = 0; i < initBoidNum; i++)
			flock.add(new Boid(this, new PVector(flockWidth/2, flockHeight/2, flockDepth/2 )));
	...
	public void draw() {
		...
		for (int i = 0; i < flock.size(); i++) {
			...
			if(scene.animatedFrameWasTriggered)
				tempBoid.run(flock, avoidWalls, flockWidth, flockHeight, flockDepth); // tell the temporary boid to execute its run method
			tempBoid.render(hue); // tell the temporary boid to execute its render method
		}
